...
export function ModernPerformanceDistribution({ studentData, data, totalStudents }: ModernPerformanceDistributionProps) {
  ...
  console.log('ModernPerformanceDistribution - Raw content:', data.content);
  console.log('ModernPerformanceDistribution - Has ### headers:', data.content.includes('###'));
  console.log('ModernPerformanceDistribution - Has ** bold:', data.content.includes('**'));

  // Enhanced markdown components with debug logging and !important styles
  const markdownComponents = {
    p: ({ children, ...props }: any) => {
      console.log('ModernPerformanceDistribution - Rendering paragraph:', children);
      return <p className="mb-4 text-sm leading-relaxed text-gray-700" {...props}>{children}</p>;
    },
    strong: ({ children, ...props }: any) => {
      console.log('ModernPerformanceDistribution - Rendering strong/bold:', children);
      return <strong className="font-semibold text-gray-900 !font-bold" {...props}>{children}</strong>;
    },
    h1: ({ children, ...props }: any) => {
      console.log('ModernPerformanceDistribution - Rendering H1:', children);
      return <h1 className="text-2xl font-bold mt-8 mb-4 text-gray-900 !font-bold !text-2xl" {...props}>{children}</h1>;
    },
    h2: ({ children, ...props }: any) => {
      console.log('ModernPerformanceDistribution - Rendering H2:', children);
      return <h2 className="text-xl font-semibold mt-6 mb-4 text-gray-900 !font-semibold !text-xl" {...props}>{children}</h2>;
    },
    h3: ({ children, ...props }: any) => {
      console.log('ModernPerformanceDistribution - Rendering H3:', children);
      return <h3 className="text-lg font-semibold mt-6 mb-3 text-gray-900 !font-semibold !text-lg" {...props}>{children}</h3>;
    },
    h4: ({ children, ...props }: any) => {
      console.log('ModernPerformanceDistribution - Rendering H4:', children);
