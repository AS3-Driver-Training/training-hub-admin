...
const sampleAnalyticsData: AnalyticsData = {
  ...
  anthropic_responses: {
    ...
    exercise_breakdown: {
      ...
      content: "### SLALOM EXERCISE\n**Group Average Vehicle Control: 85%** *(Above 80% security threshold)*  \n**Group Average Runs Until Pass: 8**\n\n**Top Performers**:\n- **Joe Zink**: 87% control, 2 attempts *(Exceptional efficiency with high control percentage)*\n- **Charlie Tompkins**: 87% control, 6 attempts *(Strong vehicle control with reasonable attempt count)*\n- **Dechase Tyler**: 86% control, 6 attempts *(Consistent performance with good control)*\n\n**Development Opportunities**:\n- **Eric Villarreal**: 85% control, 18 attempts *(Needs to reduce attempts while maintaining control)*\n- **Charles Custance**: 84% control, 17 attempts *(Required significant practice to achieve proficiency)*\n\n### BARRICADE EVASION EXERCISE  \n**Group Average Vehicle Control: 80%** *(Meets security threshold)*  \n**Group Average Runs Until Pass: 5**\n\n**Exceptional Performance**:\n- **Eric Villarreal**: 88% control, 3 attempts *(Outstanding control with efficient learning)*\n- **Charlie Tompkins**: 82% control, 2 attempts *(Quick mastery with solid control)*\n- **Chris Brown**: 83% control, 2 attempts *(Efficient learning curve with good control)*\n\n**Additional Training Beneficial**:\n- **Brian Cervantes**: 75% control, 16 attempts *(Needs focused work on vehicle control and technique)*\n- **Dechase Tyler**: 77% control, 7 attempts *(Should focus on improving control percentage)*\n\n### FINAL EXERCISE ANALYSIS\n**Group Average Performance: 89%**  \n**Average Time: 01:19.70** *(Target: 01:11.00)*\n\n**Standout Performers**:\n- **Charlie Tompkins**: 95% under high stress *(Excellent time management with minimal penalties)*\n- **Steve Sanderson**: 97% under high stress *(Superior performance under pressure)*\n- **Charles Custance**: 94% under high stress *(Strong execution with perfect accuracy)*\n\nThe analysis shows a group that generally meets or exceeds security standards, with particular strength in final exercise performance. Individual variations in learning curves suggest opportunities for targeted skill development, especially in barricade evasion consistency.",
